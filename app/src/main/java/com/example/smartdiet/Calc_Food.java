...
public class Calc_Food extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ArrayAdapter<String> adapter = new ArrayAdapter<>(this, android.R.layout.select_dialog_singlechoice, name);
        ...
        button.setOnClickListener(v -> {
            ...
            for(int i=0;i<k;i++)
            {
                itog.append(txtstr[i]).append(";");
                double Kaled=Double.parseDouble(kkal[ArrayUtils.indexOf(name,txtstr[i])])*(tetstr[i]*a/100);
                Kal=Kal+Kaled;
            }
            if(!itog.toString().equals("")) {
                Eat eat = new Eat(arguments.get("log").toString(), itog.toString(), Kal, curDate);
